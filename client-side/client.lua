...
-----------------------------------------------------------------------------------------------------------------------------------------
cRP = {}
Tunnel.bindInterface("strong_blitz", cRP)
vSERVER = Tunnel.getInterface("strong_blitz")

local cfg = module("strong_blitz","config")
...
RegisterCommand(cfg.ComandoOpen, function(source, args, rawCommand)
    if vSERVER.checkPermission(cfg.Permissao) then
        open = true
        ...
        TriggerEvent("Notify","negado","Você não tem permissão",6000)
    end
end)
